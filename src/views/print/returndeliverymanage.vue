  ...
  <div id="returndeliverymanage">
    <div class="printHome" v-show="showHome">
      <el-form :model="params" ref="queryForm" label-width="125px" class="demo-ruleForm" size="medium" id="print">
        <el-row :span="20">
          <el-col :span="6">
            <el-form-item label="退货出库单编号：" size="mini">
              <el-input v-model.trim="params.outboundNoticeCode" placeholder=""></el-input>
            </el-form-item>
          </el-col>
          <el-col :span="6">
            <el-form-item label="采购退货单编号：" size="mini">
              <el-input v-model.trim="params.purchaseOutboundOrderCode" placeholder=""></el-input>
            </el-form-item>
          </el-col>
          <el-col :span="6">
            <el-form-item label="供应商名称：" size="mini">
              <el-input v-model.trim="params.supplierName" placeholder=""></el-input>
            </el-form-item>
          </el-col>
          <el-col :span="6">
            <el-form-item label="出库单联系人：" size="mini">
              <el-input v-model.trim="params.outboundUserName" placeholder=""></el-input>
            </el-form-item>
          </el-col>
        </el-row>
        ...
        <el-row :span="20">
          <el-col :span="6">
            <el-form-item label="出库单状态：" size="mini">
              <el-select style="width: 100%;" v-model="params.outboundStatus" placeholder="请选择">
                <el-option v-for="(item, i) in outboundStatusList" :key="i" :label="item.name" :value="item.id" size='mini'>
                </el-option>
              </el-select>
            </el-form-item>
          </el-col>
          <el-col :span="6">
            <el-form-item label="退货类型：" size="mini">
              <el-select style="width: 100%;" v-model="params.returnOrderType" placeholder="请选择">
                <el-option v-for="(item, i) in returnOrderTypeList" :key="i" :label="item.name" :value="item.id" size='mini'>
                </el-option>
              </el-select>
            </el-form-item>
          </el-col>
          <el-col :span="6">
            <el-form-item label="单据类型："  size="mini">
                <el-select v-model="params.billType" filterable placeholder="请选择">
                    <el-option
      ...
      <el-table ref="tableRow" highlight-current-row @select-all="selectAll" @select="selectBox" height="400" @row-click="cellClick" border :data="tableList" style="width: 100%">
        <el-table-column align="center" fixed="left" :selectable="checkSelectable" type="selection" width="55"></el-table-column>
        <el-table-column align="center" type=index label="序号" width="55"></el-table-column>
        <el-table-column align="center" prop="outboundNoticeCode" label="退货出库单编号" show-overflow-tooltip></el-table-column>
        <el-table-column align="center" prop="purchaseOutboundOrderCode" label="采购退货单编号" show-overflow-tooltip></el-table-column>
        <el-table-column align="center" prop="supplierName" label="供应商名称" show-overflow-tooltip></el-table-column>
        <el-table-column align="center" prop="returnOrderType" label="退货类型" show-overflow-tooltip>
          <template slot-scope="scope">
            {{ typeChange(scope.row.returnOrderType) }}
          </template>
        </el-table-column>
        ...
        <el-table-column align="center" prop="pickType" label="提货方式" show-overflow-tooltip>
          <template slot-scope="scope">
            {{ checkPickType(scope.row.pickType) }}
          </template>
        </el-table-column>
        <el-table-column align="center" prop="outboundUserName" label="出库单联系人" show-overflow-tooltip></el-table-column>
        <el-table-column align="center" prop="outboundUserNumber" label="联系人电话" show-overflow-tooltip></el-table-column>
        <el-table-column align="center" prop="outboundStatus" label="出库状态" show-overflow-tooltip>
          <template slot-scope="scope">
            {{ statasflag(scope.row.outboundStatus) }}
          </template>
        </el-table-column>
        <el-table-column align="center" prop="createTime" label="系统接收时间" show-overflow-tooltip></el-table-column>
        <el-table-column class="no" align="center" fixed="right" label="操作">
          <template slot-scope="scope">
            <div @click="hideHome(scope.row.id, scope.row.allocateOrderCode)" v-if="scope.row.outboundStatus == '1'">
      ...
      <el-tabs v-model="activeTab">
        <el-tab-pane label="商品明细" name="goods">
          <el-table highlight-current-row :data="goodOutInfoListRow" border style="width: 100%">
            <el-table-column type="index" align="center" label="序号" width="50"></el-table-column>
            <el-table-column prop="skuName" align="center" label="SKU名称" show-overflow-tooltip></el-table-column>
            <el-table-column prop="skuCode" align="center" label="商品SKU编号" show-overflow-tooltip></el-table-column>
            <el-table-column prop="specNatureInfo" align="center" label="规格" show-overflow-tooltip></el-table-column>
            <el-table-column prop="barCode" align="center" label="条形码" show-overflow-tooltip></el-table-column>
            <el-table-column prop="brandName" align="center" label="品牌名称" show-overflow-tooltip></el-table-column>
            <el-table-column align="center" label="库存类型">
              <template slot-scope="scope">
                {{ typeChange(scope.row.returnOrderType ) }}
              </template>
            </el-table-column>
            <el-table-column prop="returnQuantity" align="center" label="计划退货数量"></el-table-column>
            <el-table-column prop="actualStorageQuantity" align="center" label="实际出库数量" @cell-class-name='addTip'></el-table-column>
          </el-table>
        </el-tab-pane>
        <el-tab-pane label="收货信息" name="sender">
          <el-row :gutter="20" style="font-size: 14px">
            ...
            <el-col :span="6">
              收货人手机号：{{otherInfoDetailRow.receiverNumber}}
            </el-col>
            <el-col :span="6">
              收货省份.城市.区域：
              <span v-if="otherInfoDetailRow.receiverProvince">{{otherInfoDetailRow.receiverProvince}}</span>
              <span v-if="otherInfoDetailRow.receiverCity">.{{otherInfoDetailRow.receiverCity}}</span>
              <span v-if="otherInfoDetailRow.receiverArea">.{{otherInfoDetailRow.receiverArea}}</span>
            </el-col>
        ...
        <el-tab-pane label="其他信息" name="otherInfo">
          <el-row>
            <el-col style="margin-bottom: 15px;">
              <p style="display: inline-block; width: 300px;">退货说明：{{otherInfoDetailRow.returnPolicy}}</p>
              <p style="display: inline-block; width: 300px;">退货备注：{{otherInfoDetailRow.remark}}</p>
            </el-col>
            <el-col style="margin-bottom: 15px;">
              <p style="display: inline-block; width: 300px;">运输物流：{{otherInfoDetailRow.logisticsCorporation}}</p>
              <p style="display: inline-block; width: 300px;">运单号：{{otherInfoDetailRow.waybillNumber}}</p>
            </el-col>
            <el-col>
              出库备注：{{otherInfoDetailRow.outRemark}}
            </el-col>
          </el-row>
        </el-tab-pane>
    ...
    <div class="printChild" v-show="!showHome">
      <div class="secondHomeHead">
        ...
        <div class="content" style="border: none">
          <div>
            <p>
              <span class="title">退货出库单编号：</span>{{otherInfoDetail.outboundNoticeCode}}</p>
            <p>
              <span class="title">采购退货单编号：</span>{{otherInfoDetail.purchaseOutboundOrderCode}}</p>
            <p>
              <span class="title">供应商名称：</span>{{otherInfoDetail.supplierName}}</p>
            <p>
              <span class="title">提货方式：</span>{{checkPickType(otherInfoDetail.pickType)}}</p>
          </div>
          <div>
            <p>
              <span class="title">收货人：</span>{{otherInfoDetail.receiver}}</p>
            <p>
              <span class="title">收货人手机号：</span>{{otherInfoDetail.receiverNumber}}</p>
            <p>
              <span class="title">收货省份.城市.区域：</span>
              <span v-if="otherInfoDetail.receiverProvince">{{otherInfoDetail.receiverProvince}}</span>
              <span v-if="otherInfoDetail.receiverCity">.{{otherInfoDetail.receiverCity}}</span>
              <span v-if="otherInfoDetail.receiverArea">.{{otherInfoDetail.receiverArea}}</span>
            </p>
            <p>
          ...
          <div>
            <p>
              <span class="title">单据类型：</span>{{checkedBillType(otherInfoDetail.billType)}}</p>
            <p></p>
            <p></p>
            <p></p>
          </div>
      ...
      <div class="search_outparams">
        ...
        <p>
          ...
          <span class="desc">
            <el-input v-model="outParams.waybillNumber" placeholder="" size='mini'></el-input>
          </span>
      ...
      <div id="tableMain">
          ...
          <el-table highlight-current-row :data="goodOutInfoList" border class="tiptable" style="width: 100%">
            ...
            <el-table-column property="skuCode" label="SKU编号" align="center" show-overflow-tooltip>
            </el-table-column>
            <el-table-column property="skuName" label="SKU名称" align="center" show-overflow-tooltip>
            </el-table-column>
            <el-table-column property="specNatureInfo" label="规格" align="center" show-overflow-tooltip>
            </el-table-column>
            <el-table-column property="barCode" label="条形码" align="center" show-overflow-tooltip>
            </el-table-column>
            <el-table-column label="退货类型" align="center">
            <template slot-scope="scope">
                {{ typeChange(scope.row.returnOrderType ) }}
            </template>
            </el-table-column>
            <el-table-column property="returnQuantity" label="计划退货数量" align="center">
            </el-table-column>
            <el-table-column label="实际出库数量" align="center">
            <template slot-scope="scope">
                <el-input v-model="arr[scope.$index]" placeholder="" size="mini" @change="inputChange(scope.$index, scope.row)" maxlength="10"></el-input>
            </template>
            </el-table-column>
        </el-table>
      </div>
      ...
      <div class="second-home-footer">
        ...
        <el-button size="mini" @click="backHome">返回</el-button>
    ...
    <div class="print_screen" v-for="(data, i) in printData" :key="i" v-show="isListShow">
      ...
      <section v-if="data">
        <p>
          ...
          <span class="desc">{{data.outboundNoticeCode}}</span>
        </p>
        <p></p>
        <p></p>
        <p></p>
      ...
      <section v-if="data">
        <p>
          <span class="title">采购退货单编号：</span>
          <span class="desc">{{ data.purchaseOutboundOrderCode}}</span>
        </p>
        <p>
        ...
        <p>
          <span class="title">退货类型：</span>
          <span class="desc">{{ typeChange(data.returnOrderType) }}</span>
        </p>
        <p>
          <span class="title">出库单联系人：</span>
          <span class="desc">{{ data.outboundUserName }}</span>
        </p>
      </section>
      <section v-if="data">
        <p>
        ...
        <p style="width: 49%">
          <span class="title">收货方省份.城市.地区：</span>
          <span class="desc">{{ data.receiverProvince}}.{{ data.receiverCity }}.{{ data.receiverArea }}</span>
        </p>
      </section>
      <p class="name">商品明细</p>
      <div class="table_list">
        <el-table highlight-current-row :data="data.itemInfoVOList" style="width: 100%; border: solid 1px #ccc;" border>
          ...
          <el-table-column prop="returnQuantity" label="计划退货数量" align="center"></el-table-column>
          <el-table-column prop="" label="拣货情况" align="center"></el-table-column>
        </el-table>
      </div>
    </div>
...
export default {
  data() {
    return {
      ...
      goodOutInfoListRow: [], // 商品出库信息列表 明细
      ...
      params: {
        billType: 0, // 单据类型
        outboundNoticeCode: '', // 退货出库单编号
        purchaseOutboundOrderCode: '', // 采购退货单编号
        supplierName: '', // 供应商名称
        outboundUserName: '', // 出库单联系人
        warehouseCode: '', // 仓库编号
      ...
      logParams: {
        ...
        pageSize: 10,
        pageNumber: 1,
        entityType: 'WmsPurchaseOutboundOrder',
        entityId: ''
      },
      logTotal: 0, // 日志总条数
      goodsInfo: [], //货物详情
  ...
  methods: {
    ...
    operatData(str) {
        if(str == 'fill'){
            this.goodOutInfoList.map((item, index) => {
                this.arr[index] = item.returnQuantity
                this.$set(this.arr, index, item.returnQuantity)
            })
        }else if(str == 'clear'){
            this.arr = []
        }
    ...
    needPrintData(select) {
      ...
      let promise = select.map((item, index) => {
        ...
        // let json = item;
        let json = {};
        let code = item.outboundNoticeCode;
        return this.GET('/wmsReturnOrderManager/queryReturnOrderDetail/' + code + '/1').then(res => {
    ...
    save() {
      ...
      this.goodOutInfoList.forEach((item, i) => {
        ...
        obj.actualStorageQuantity = this.arr[i];
        paramsArr.push(obj);
      });
      // 查找公司名字
      if (this.outParams.logisticsCorporationCode) {
        this.companyList.forEach(item => {
          if (item.logisticsCode === this.outParams.logisticsCorporationCode) {
            this.outParams.logisticsCorporation = item.logisticsCorporationName; // 查找公司名字
          }
        });
      }
      this.outParams.outboundNoticeCode = this.otherInfoDetail.outboundNoticeCode; // 主键
      // this.outParams.id = this.id; // 主键
      this.outParams.outRemark = this.textarea; // 出库备注
      this.outParams.saveFlag = '1'; // 操作标记保存 1
      this.outParams.itemInfoVOList = paramsArr;
      let Reg = /^[0-9]+$/;
      let isInt = false;
      if (this.outParams.itemInfoVOList.length) {
        this.outParams.itemInfoVOList.forEach(item => {
          if ((item.actualStorageQuantity < 0 || !Reg.test(item.actualStorageQuantity)) && item.actualStorageQuantity) {
            isInt = true;
            console.log(11111);
            return;
          }
      ...
      this.POST('/wmsReturnOrderManager/saveReturnOrderOutBound', this.outParams).then(res => {
        if (res.code == '200') {
          this.$message({
            type: 'success',
            message: '保存调拨出库单详情成功!'
    ...
    resetForm() {
      ...
      //   }
      // }
      this.params.warehouseCode = localStorage.code;
      this.needData(this.params);
    },
    ...
    cellClick(row, event, column) {
      ...
      this.code = row.outboundNoticeCode;
      this.id = row.id;
      this.logParams.entityId = row.id;
      console.log(row.outboundNoticeCode);
      console.log(row.id);
      this.goodsInfo = [];
      if(text == '出库结果录入'){
    ...
    saveAll() {
      ...
      //   return;
      // }
      // if (!this.outParams.waybillNumber) {
      //   this.$message({
      //     message: '请输入运单编号!',
      //     type: 'warning'
      //   });
      //   return;
      // }
      if (this.outParams.waybillNumber && this.outParams.waybillNumber.length > 100) {
        this.$message({
      ...
      var reg = /^[a-zA-Z0-9,]+$/;
      if (this.outParams.waybillNumber && !reg.test(this.outParams.waybillNumber)) {
        this.$message({
      ...
      var hasComma = this.outParams.waybillNumber.indexOf(',');
      var isRepet = false;
      console.log(hasComma);
      if (hasComma > -1) {
        let arr = this.outParams.waybillNumber.split(',');
        let list = [];
        console.log(arr);
        arr.forEach(item => {
      ...
      this.goodOutInfoList.forEach((item, i) => {
        let obj = {};
        obj.skuCode = item.skuCode;
        obj.returnQuantity = this.goodOutInfoList[i].returnQuantity;
        obj.actualStorageQuantity = this.arr[i];
        paramsArr.push(obj);
      });
      // 查找公司名字
      ...
      this.outParams.outboundNoticeCode = this.otherInfoDetail.outboundNoticeCode; // 主键
      this.outParams.outRemark = this.textarea; // 出库备注
      this.outParams.saveFlag = '2'; // 操作标记保存 1
      this.outParams.itemInfoVOList = paramsArr;
      ...
      if (this.outParams.itemInfoVOList.length) {
        this.outParams.itemInfoVOList.forEach(item => {
          if (item.actualStorageQuantity < item.returnQuantity) {
            lessArr.push(item.skuCode);
          }
          if (item.actualStorageQuantity === '' || item.actualStorageQuantity === 'undefined' || item.actualStorageQuantity === null) {
            this.$message({
              message: '请输入实际出库数量!',
              type: 'warning'
            });
            isShow = false;
            return;
          } else if (item.actualStorageQuantity < 0 || !Reg.test(item.actualStorageQuantity)) {
            this.$message({
              message: '实际出库数量只能为0或者正整数!',
              type: 'warning'
            });
          ...
          // else if (item.actualStorageQuantity > item.returnQuantity) {
          //   isMore = true;
          //   return;
          // }
        });
      ...
      if (isLess) {
        if (!this.textarea) {
          this.$message({
            message: '存在SKU的实际出库数量不等于计划调拨数量，请填写"出库备注"!',
            type: 'warning'
          });
          return;
        }
      ...
      if (this.outParams.itemInfoVOList.length) {
        ...
        this.outParams.itemInfoVOList.forEach(item => {
          ...
          params.actualStorageQuantity = item.actualStorageQuantity;
          list.push(params);
        });
        this.outParams.itemInfoVOList = list;
      }
      console.log('出库参数2：', this.outParams);
      if (isShow && !isMore) {
        this.$confirm('确定后将不能再修改数据!', `${result}您确定“出库完成”吗？`, {
          ...
          .then(() => {
            ...
            this.POST('/wmsReturnOrderManager/saveReturnOrderOutBound', this.outParams, true).then(res => {
              if (res.code == '200') {
                this.$message({
                  type: 'success',
                  message: '出库完成!'
    ...
    needData(params) {
      //获取调拨入库列表
      return this.GET('/wmsReturnOrderManager/queryReturnOrderPage', params).then(res => {
        ...
        if (this.tableList[0]) {
          ...
          let code = this.tableList[0].outboundNoticeCode;
    ...
    // 获取调拨入库单个列表的详情 明细
    needTableDatailRow(outboundNoticeCode, load) {
        this.goodsInfoRow = []
        this.otherInfoDetailRow = {}
        this.goodOutInfoListRow = []
        this.GET('/wmsReturnOrderManager/queryReturnOrderDetail/'+outboundNoticeCode+'/0','',load).then(res => {
                this.goodsInfoRow = res.body;
                this.otherInfoDetailRow = res.body;
                this.goodOutInfoListRow = this.goodsInfoRow.itemInfoVOList;
        })
    },
    // 获取调拨入库单个列表的详情
    needTableDatail(outboundNoticeCode, load) {
        this.goodsInfo = {}
        this.otherInfoDetail = {}
        this.goodOutInfoList = []
        this.GET('/wmsReturnOrderManager/queryReturnOrderDetail/'+outboundNoticeCode+'/0','',load).then(res => {
                this.goodsInfo = res.body;
                this.otherInfoDetail = res.body;
                this.goodOutInfoList = this.goodsInfo.itemInfoVOList;
                this.goodOutInfoList.forEach(item => {
                if (item.actualStorageQuantity || item.actualStorageQuantity === 0) {
                    this.arr.push(item.actualStorageQuantity);
                }
                });
                if (this.otherInfoDetail.waybillNumber) {
                    this.outParams.waybillNumber = this.otherInfoDetail.waybillNumber;
                }
                ...
                if (this.otherInfoDetail.outRemark) {
                    this.textarea = this.otherInfoDetail.outRemark;
                }
                if (this.otherInfoDetail.pickType == '2') {
                    this.outParams.logisticsCorporationCode = 'KD031';
                }
        })
    //   axios({
    //     methods: 'get',
    //     url: '/wms-web' + '/wmsReturnOrderManager/queryReturnOrderDetail/' + outboundNoticeCode + '/0'
    //   }).then(res => {
    ...
    getCompanyList() {
      axios({
        method: 'get',
        url: '/wms-web' + '/logisticsCorporation/all'
      }).then(res => {
